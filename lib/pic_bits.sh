...
if ! cpp_output="$(cpp -dM $1)"; then
    echo "Unable to read elf header file" >&2
    echo "Ensure glibc-headers is installed" >&2
    echo "On macOS, ensure libelf is installed via brew" >&2
    exit 1
fi

if [ -z "$1" ] && [ -z "$2" ]; then
    echo "Usage: $(basename $0) [path to elf header] [output file]" >&2
    exit 1
fi

echo "/*" > $2
echo " * This file is autogenerated. DO NOT EDIT." >> $2
echo " * See pic_bits.sh to modify." >> $2
echo " */" >> $2
echo "#include <stdbool.h>" >> $2
echo "#include <stdio.h>" >> $2
echo "#ifdef _LINUX" >> $2
echo "#include <elf.h>" >> $2
echo "#endif" >> $2
echo "#ifdef _DARWIN" >> $2
echo "#include <libelf.h>" >> $2
echo "#endif" >> $2
echo "bool is_pic_reloc(Elf64_Half machine, Elf64_Xword rel_type)" >> $2
echo "{" >> $2
echo "    switch (machine) {" >> $2
...
echo "$cpp_output" | sed -n -E 's/^#define[[:space:]]+(EM_[^[:space:]]+).*/\1/p' | \
    while read -r arch; do
        ...
        if [ -n "$relocs" ]; then
            echo "        case ${arch}:" >> $2
            echo "            switch (rel_type) {" >> $2
            echo "$relocs" | while read -r reloc_type ; do
                echo "                case ${reloc_type}:" >> $2
            done
            echo "                    return true;" >> $2
            echo "                default:" >> $2
            echo "                    return false;" >> $2
