...
# Provides dependency is correct (OK) - control case
class ProvidesCorrectRPMs(TestRPMs):
    def setUp(self):
        super().setUp()

        self.rpm.add_provides(after_provides)
...
class RetainingProvidesRebaseCompareRPMs(TestCompareRPMs):
    def setUp(self):
        super().setUp(rebase=True)

        self.before_rpm.add_provides(after_provides)
...
class RetainingProvidesRebaseCompareKoji(TestCompareKoji):
    def setUp(self):
        super().setUp(rebase=True)
...
# Retaining Provides dependency in maint comparison (OK)
class RetainingProvidesCompareRPMs(TestCompareRPMs):
    def setUp(self):
        super().setUp()
...
class GainingProvidesRebaseCompareRPMs(TestCompareRPMs):
    def setUp(self):
        super().setUp(rebase=True)
...
class GainingProvidesRebaseCompareKoji(TestCompareKoji):
    def setUp(self):
        super().setUp(rebase=True)
...
class ChangingProvidesRebaseCompareRPMs(TestCompareRPMs):
    def setUp(self):
        super().setUp(rebase=True)
...
class ChangingProvidesRebaseCompareKoji(TestCompareKoji):
    def setUp(self):
        super().setUp(rebase=True)
...
class ChangingProvidesExpectedCompareSRPM(TestCompareSRPM):
    def setUp(self):
        super().setUp()
...
class ChangingProvidesExpectedCompareKoji(TestCompareKoji):
    def setUp(self):
        super().setUp()
...
# Losing a Provides in a rebase comparison (INFO)
class LosingProvidesRebaseCompareRPMs(TestCompareRPMs):
    def setUp(self):
        super().setUp(rebase=True)
...
class LosingProvidesRebaseCompareKoji(TestCompareKoji):
    def setUp(self):
        super().setUp(rebase=True)
...
# otherwise)
class MissingEpochProvidesSRPM(TestSRPM):
    def setUp(self):
        super().setUp()
        ...
        # manually add a devel subpackage with an incorrect Provides
        sub.add_provides("%s = %s-%s" % (AFTER_NAME, AFTER_VER, AFTER_REL))
...
class MissingEpochProvidesCompareSRPM(TestCompareSRPM):
    def setUp(self):
        super().setUp()
        ...
        # manually add a devel subpackage with an incorrect Provides
        before_sub.add_provides("%s = %s-%s" % (BEFORE_NAME, BEFORE_VER, BEFORE_REL))
        after_sub.add_provides("%s = %s-%s" % (AFTER_NAME, AFTER_VER, AFTER_REL))
...
class MissingEpochProvidesRebaseCompareSRPM(TestCompareSRPM):
    def setUp(self):
        super().setUp(rebase=True)
...
class MissingEpochProvidesRebaseCompareKoji(TestCompareKoji):
    def setUp(self):
        super().setUp(rebase=True)
...
# Unexpanded macro in Provides (BAD)
class UnexpandedMacroProvidesSRPM(TestSRPM):
    @unittest.skipIf(on_alt_linux, "ALT Linux rpmbuild prohibits unexpanded macros")
    def setUp(self):
        super().setUp()

        self.rpm.add_provides(unexpanded_provides)
...
class UnexpandedMacroProvidesKoji(TestKoji):
    @unittest.skipIf(on_alt_linux, "ALT Linux rpmbuild prohibits unexpanded macros")
    def setUp(self):
        super().setUp()
...
class UnexpandedMacroProvidesCompareSRPM(TestCompareSRPM):
    @unittest.skipIf(on_alt_linux, "ALT Linux rpmbuild prohibits unexpanded macros")
    def setUp(self):
        super().setUp()

        self.before_rpm.add_provides(unexpanded_provides)
        self.after_rpm.add_provides(unexpanded_provides)
...
class UnexpandedMacroProvidesCompareKoji(TestCompareKoji):
    @unittest.skipIf(on_alt_linux, "ALT Linux rpmbuild prohibits unexpanded macros")
    def setUp(self):
        super().setUp()
...
class MissingExplicitProvidesCompareSRPM(TestCompareSRPM):
    def setUp(self):
        super().setUp()
