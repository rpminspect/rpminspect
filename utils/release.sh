...
if [ "${OPT_GITHUB}" = "y" ]; then
    ...
    API_JSON=$(jq -ns --arg tag_name "${TAG}" --arg target_commitish "master" --arg name "${PROJECT}-${VERSION}" --arg body "$(git log --format="%s" v0.8 | sed -e 's|^|* |g')" '{ tag_name: $tag_name, target_commitish: $target_commitish, name: $name, body: $body, draft: false, prerelease: false }')
    RELEASE_INFO="$(mktemp)"
